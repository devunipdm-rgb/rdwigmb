...
async function startCampaign(campanhaId, listaContatos, mensagem, imagemBase64, intervaloGeral = 60, intervaloPausa = 30) {
    ...
    console.log(`⏱️  Intervalo geral: ${intervaloGeral}s | Pausa: ${intervaloPausa}s`);
    ...
    for (let i = 0; i < listaContatos.length; i++) {
        ...
        try {
            ...
            // Delay anti-ban: aleatório entre 15s e intervaloGeral, com pausa a cada 10 mensagens

            const intervaloMin = 15000; // 15 segundos mínimo

            const intervaloMax = Math.max(intervaloMin, intervaloGeral * 1000);

            let tempoEspera = Math.floor(Math.random() * (intervaloMax - intervaloMin + 1) + intervaloMin);

            

            // Pausa adicional a cada 10 mensagens

            if ((i + 1) % 10 === 0 && i > 0) {

                tempoEspera += intervaloPausa * 1000;

                console.log(`⏸️  Pausa de ${intervaloPausa}s após ${i + 1} mensagens`);
...
app.get('/qrcode', async (req, res) => {
    try {
        ...
        if (!sock) {
            ...
            let tentativas = 0;
            const maxTentativas = 30; // 30 * 500ms = 15 segundos
            
            while (tentativas < maxTentativas && !lastQR) {
...
app.post('/disparar', async (req, res) => {
    ...
    const { campanhaId, contatos, mensagem, imagem, intervaloGeral, intervaloPausa } = req.body;
    ...
    // Iniciar campanha em background (passando imagem se existir e configurações)
    startCampaign(campanhaId, contatos, mensagem || '', imagem, intervaloGeral, intervaloPausa).catch(err => {
